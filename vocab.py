...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            ...
            for word in sent:
                word_id_cur =  []
                word_id_cur.append(self.start_of_word)
                for char in word:
                    word_id_cur.append(self.char2id[char])
                word_id_cur.append(self.end_of_word)
                word_ids_cur.append(word_id_cur)
            word_ids.append(word_ids_cur)
        return word_ids
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        paddedSents = pad_sents_char(charIndices,self['<pad>'])
        tensorPadded = torch.tensor(paddedSents,dtype=torch.long, device=device)
        return tensorPadded.permute(1,0,2)
        ### END YOUR CODE
